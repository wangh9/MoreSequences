...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(75.0, 50.0),  # All numbers are approximate.
                rg.Point(37.5, 71.7),
                rg.Point(37.5, 28.3),
                ]
    circle = rg.Circle(rg.Point(50, 50),
                       25)
...
def run_test_draw_points_on_circle():
    ...
    draw_points_on_circle(window, circle, 7, 'green')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    i = []
    i = generate_points_on_circle(circle,number_of_points)
    for k in range (number_of_points):
        i[k].attach_to(window)
        cir1 = rg.Circle(i[k],10)
...
def run_test_pizza():
    ...
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 100), 150)
    circle.outline_thickness = 3
    pizza(window, circle, 20, 'yellow', 3)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    i = []
    i = generate_points_on_circle(circle, number_of_slices)
    for k in range(number_of_slices):
        i[k].attach_to(window)
        lin1 = rg.Line(i[k], circle.center)
        lin1.color = color
        lin1.thickness =thickness
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON tests 3 segments' + \
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.attach_to(window)
    i = []
    i = generate_points_on_circle(circle, number_of_segments)
    for k in range(number_of_segments-1):
        i[k].attach_to(window)
        lin1 = rg.Line(i[k], i[k+1])
        lin1.color = color
        lin1.thickness = thickness
        lin1.attach_to(window)
    lin2 = rg.Line(i[len(i)-1],i[0])
    lin2.color = color
    lin2.thickness = thickness
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'FANCY POLYGON test 4:  10 blue lines' + \
            ' on green circle, hops = 6.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'green'
    fancy_polygon(window, circle, 10, 6, 'blue', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines,
    ...
    for k in range (len(i)//2):
        li1 = rg.Line(i[k], i[k+hops_to_next_point])
        li1.color = color
        li1.thickness = thickness
        li1.arrow = "last"
        li1.attach_to(window)
    window.render()
# ----------------------------------------------------------------------
